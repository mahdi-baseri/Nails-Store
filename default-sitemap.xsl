...
>
	...
	<xsl:template match="/">
		...
		<html xmlns="http://www.w3.org/1999/xhtml">
			<head>
				<title>
					<xsl:choose>
						<xsl:when test="$fileType='Sitemap' or $fileType='RSS'">نقشه</xsl:when>
						<xsl:otherwise>شاخص نقشه سایت ویدئو</xsl:otherwise>
					</xsl:choose>
				</title>
		...
		.breadcrumb svg.back {
			...
			<body>
				...
				<xsl:call-template name="Header">
					<xsl:with-param name="title">نقشه</xsl:with-param>
	...
	<a href="/"><span>صفحه اصلی</span></a>
			...
			<svg width="6" height="8" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M.727 7.06L3.78 4 .727.94l.94-.94 4 4-4 4-.94-.94z" fill="#141B38"/></svg>
					...
					<span>شاخص نقشه سایت ویدئو</span>
	...
	<a href="/"><span>صفحه اصلی</span></a>
			...
			<svg width="6" height="8" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M.727 7.06L3.78 4 .727.94l.94-.94 4 4-4 4-.94-.94z" fill="#141B38"/></svg>
					...
					<a href="/sitemap.xml"><span>شاخص نقشه سایت ویدئو</span></a>
						<svg width="6" height="8" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M.727 7.06L3.78 4 .727.94l.94-.94 4 4-4 4-.94-.94z" fill="#141B38"/></svg>
	...
	<div id="content-head">
		...
		<xsl:choose>
			<xsl:when test="$fileType='RSS'">
				<p>تولید شده توسط <a href="https://aioseo.com/?utm_source=WordPress&#038;utm_campaign=liteplugin&#038;utm_medium=xml-sitemap" target="_blank" rel="noreferrer noopener">All in One SEO</a>, this is an RSS Sitemap, meant to be consumed by search engines like Google or Bing.</p>
				<p>
			...
			<xsl:otherwise>
				<p>تولید شده توسط <a href="https://aioseo.com/?utm_source=WordPress&#038;utm_campaign=liteplugin&#038;utm_medium=xml-sitemap" target="_blank" rel="noreferrer noopener">All in One SEO</a>, این یک نقشه سایت XML است ، که توسط موتورهای جستجو مانند گوگل یا بینگ مورد استفاده قرار می گیرد.</p>
				<p>
					می توانید اطلاعات بیشتر در مورد نقشه سایت XML را در <a href="https://www.sitemaps.org/" target="_blank" rel="noreferrer noopener"> sitemaps.org </a> بیابید.				</p>
			</xsl:otherwise>
		</xsl:choose>
			...
			<p>
				<xsl:choose>
					<xsl:when test="$fileType='Sitemap' or $fileType='RSS'">
						این نقشه سایت شامل						<xsl:value-of select="$amountOfURLs"/>
						<xsl:choose>
					...
					<xsl:otherwise>
						این فهرست نقشه سایت شامل						<xsl:value-of select="$amountOfURLs"/>
						<xsl:choose>
							<xsl:when test="$amountOfURLs = 1">
								سایت مپ							</xsl:when>
							<xsl:otherwise>
								نقشه‌های سایت							</xsl:otherwise>
						</xsl:choose>
					</xsl:otherwise>
				</xsl:choose>
				و بر روی یک آگوست 12, 2025 تولید 8:54 ق.ظ			</p>
		</xsl:if>
	</div>
</xsl:template>
	...
	<xsl:variable name="priorityLabel">
		<xsl:choose>
			<xsl:when test="$priorityLevel = 'low'">کم</xsl:when>
			<xsl:when test="$priorityLevel = 'medium'">متوسط</xsl:when>
			<xsl:when test="$priorityLevel = 'high'">زیاد</xsl:when>
		</xsl:choose>
	</xsl:variable>
